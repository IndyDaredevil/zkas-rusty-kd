...
3. **Reset execution** — regen genesis with the full bundle, cut the binary, relaunch order **node → wallets/walletd → pool/bridge**.
   - **Port migration (baked in):** ZKas now defaults to its own port block — gRPC **16810**, P2P **16811**, wRPC borsh **17810**, json **18810** (mainnet) — distinct from Kaspa's 161xx-166xx so the node and the merged-mining Kaspa parent coexist with no overrides. At deploy, update every explicit `:16110`/`:17110` reference: the public-gRPC socat (185.147.157.125:16110→:16810), nginx upstreams, the VPS2 tunnel (`-L 8501/8500` stay; node RPC repoint), walletd/api `-s`, the pool bridge `zkas-bridge.yaml` node address, and `--addpeer` (16111→16811). The Kaspa parent can move back to its real 16110/16111.
