# Desktop: connects to the public node at process start (before unlock); "your own node" reverts to "Set up" every launch, node left stopped, run-mode dialog defaults to Mining

**Not a re-open of #1; different area (desktop chain-source chooser).**

## Environment
- ZKas Wallet desktop **1.0.29**, macOS aarch64 (ad-hoc DMG). Will confirm on 1.0.32 in a comment.
...
1. Launch from the Dock. The Unlock screen appears. **Before the PIN is entered**, lsof already shows two ESTABLISHED connections from the desktop process to the public node:
   `zkas-desk … 192.168.1.173:54034->185.147.157.125:16110 (ESTABLISHED)`
   `zkas-desk … 192.168.1.173:54033->185.147.157.125:16110 (ESTABLISHED)`
...
Happy to test a build or bisect between 1.0.19 and 1.0.29.
